...
// Razorpay's independent server-side confirmation/recovery path — the
// browser's own post-Checkout callback (app/checkout/razorpay-actions.ts)
...
// callback simply never runs. This is a Route Handler, not a Server
// Action, because it must be a plain public HTTP endpoint Razorpay's
// servers can POST to directly with no Supabase session at all.
//
...
// Application code should only ever hit the exported POST handler below.
